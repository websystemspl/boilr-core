...
class ActionsYamlFileLoader extends FileLoader
{
    private $yamlParser;
    ...
    private function parseDefinitions(array $content, string $file)
    {
        ...
        if (!\is_array($content['actions'])) {
            throw new InvalidArgumentException(sprintf('The "actions" key should contain an array in "%s". Check your YAML syntax.', $file));
        }

        foreach ($content['actions'] as $id => $action) {
    ...
    private function validate($content, string $file): ?array
    {
        ...
        foreach ($content as $namespace => $data) {
            ...
            throw new InvalidArgumentException(sprintf('The file "%s" is not valid. It should contain "actions". Check your YAML syntax.', $file));
        }
